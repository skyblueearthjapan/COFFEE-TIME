"""人狼ゲームの文言・定数データ（JSON）を ESP32 ファーム用の C++ ソースへ変換する。

使い方:
  python tools/gen_game_data.py
  python tools/gen_game_data.py --content <content.ja.json> --rules <rules.json> --out-dir <出力先>
...
def gen_header(content: dict, rules: dict) -> str:
    lines = []
    a = lines.append
...
def main():
    ...
    header_text = gen_header(content, rules)
