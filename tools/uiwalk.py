...
def show(raw: bytes) -> None:
    ...
    if text.startswith(("[DUEL]", "[REV]", "[CARDS]", "[ESP]", "[GAS]", "[DEV]", "[CUP]")) and " view=" not in text:
...
for step in steps:
    ...
    if kind == "key":
        ...
        while time.time() < deadline:
            ...
            if " view=" not in seen:
                if seen.startswith(("[DUEL]", "[REV]", "[CARDS]", "[ESP]", "[GAS]", "[DEV]", "[CUP]")):
        ...
        while time.time() < deadline:
            ...
            if line.startswith(("[DUEL]", "[REV]", "[CARDS]", "[ESP]", "[GAS]", "[DEV]", "[CUP]")):
