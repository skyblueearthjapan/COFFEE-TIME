...
最終更新: 2026-09-21（午後）/ 段階 **W1 完了・W2 実装済み（公開画面は実機確認済み。秘密の表示以降は人の指での確認待ち）**
...
4. 開発用シリアルコマンドで画面を直接開ける: `1`=メニュー / `2`=ゲーム一覧 / `3`=人数選択 / `0`=HOME へ戻る
   ...
   `G`=人狼の公開状態を表示 / `P<x>,<y>`=その座標をタップ（秘密は開けない）。
   画面を順に撮るには `python tools/uiwalk.py COM8 out key:3 wait:1 snap:lobby tap:240,367 snap:brief1 …`
...
## 1.5 W2 の現状（2026-09-21 午後）

実装したファイル（`firmware/src/app/games/werewolf/`）:
- `WerewolfGame.*` … 1 画面の中身を局面ごとに作り直す方式のコントローラー。20ms の LVGL タイマーで進行・タイマー・覗き見防止を回す
- `WerewolfPort.*` … `Port` の実装（時刻 = `esp_timer`、乱数 = `esp_random()`、バックライト、中立画面の確定、NVS `ct_wolf/meta`）
- `lvgl_v8_port.*` … タッチの生データ（時刻・座標・点数）を読み取りのたびに保存。GT911 を読むのは従来どおり 1 か所だけ
- `tools/gen_game_data.py` … `layout.json` の座標も C++ に出力（`coffee::wolf::layout`）

実機で確認できたこと（`tools/uiwalk.py` による自動操作とスクリーンショット）:
- [x] ロビー（人数 3〜10、**前回の人数を NVS から復元**）・遊び方 17 ページ・約束 4 ページ・人数確認 2 ページ
- [x] 配役 → 手渡し → 役職確認の画面（伏せた状態）。見ずに「覚えました」を押すと案内が出て進めない
- [x] 一時停止メニュー → コーヒーを記録（HOME の TODAY/LEFT と SD ログに反映）→ 無効にして終了 → HOME へ戻る
- [x] 自動タップ（開発用 `P<x>,<y>`）では秘密は表示されない（生のタッチデータを更新しないため）

**人の指でしか確認できず、まだ確認していないこと**（ユーザーに依頼中）:
- [ ] 押している間だけ役職が出る／離した瞬間に消える／残像が無い（動画で確認）
- [ ] 2 本指・8 秒押し続け・画面外へ指をずらす → いずれも隠れること
- [ ] 夜の対象選択 → 夜の確認 → 朝 → 議論タイマー（+1 分・投票へ）→ 投票 → 決選 → 結果の全ページ
- [ ] 2 局続けて遊べること／10 人でのページ送り／ゲーム中の電源断 → 次回「無効な局」の案内

2026-09-21 夕方の追加（ユーザー要望）: 席ごとのキャラクター（アイコン＋名前）、物語の導入 6 ページ、役職の絵、秘密の本文がボタンに重なる不具合の修正。公開画面（導入・一覧・手渡し）は実機で確認済み。**役職の絵と重なりの修正は人の指でしか見られないのでユーザー確認待ち**

W2 で**入れていないもの**: ブザー（§2-4）、一時停止中に HOME へ戻る導線、結果画面の 60 秒自動クローズ、議論時間の手動設定、演出文（intro/ending variants）

## 2. W2（本体の実装）— 当初の計画（上の現状と見比べること）
...
- **喫茶「余白」の事件簿（探偵）**: 1 人用・全 3 話。AI なしでも成立する。必要文字は 474 字で、人狼のフォントに無いのは **166 字**（「1293」はバイト数の読み違いだった）。
...
- [x] **人数の初期値**: 前回の人数を覚える方式で実装（設計書の `PublicMeta.players`。初回は 3 人）。ユーザーへ報告済み
