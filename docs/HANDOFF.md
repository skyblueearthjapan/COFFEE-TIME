...
0. **会社の Wi-Fi で PC と端末が同じネットワークにいれば、USB は要らない**（9/24〜）。下の `COM8` をすべて `net` に置き換えてよい（例 `python tools/serlog.py net 10 send=GDVUJK`）。書き換えは `python -m platformio run -e app` のあと `python tools/ota.py`。名前が引けないときは「システム情報」の IP アドレス。詳しくは §8「遠隔コンソールと Wi-Fi 越しの更新」
...
- **9/24 の作業内容**: 「残り 3 のメールが来ない」→ GAS・メール送信は正常（見本メールで確認）、ユーザーの受信箱に遅れて届いていた（原因は未特定。端末の再送 30 秒ごと or Google の配信遅延）。
  ...
  独立レビュー（承認・修正 2 件: 無線更新後の再起動が「見張り」になっていた → 再起動前に `WiFi.disconnect(true)` で「software」に / `s_game_active` を volatile）→ 無線更新で反映・確認 → コミット `768de82`・push
  実機で確かめたこと: `ping coffee-time.local` が通る / `serlog.py net` で 6 ゲームの状態 / `snapshot.py net`（約 13 秒）/ `uiwalk.py net` でシステム情報の IP の行 / 無線更新 2 回（各約 22 秒、杯数そのまま、2 回目は `reset=software`）/ 合言葉違いは `[CON] denied` / 探偵の画面を開いたままの更新は `[OTA] busy: a game screen is open`。
  未確認: 3 回まちがえたときの 60 秒の締め出し、2 台目の PC による入れ替わり、更新の途中で Wi-Fi が切れたときの復帰、`settime.py net`
  そのほか: SD の操作ログで、9/24 朝に ＋1 が**1 秒の間に 5 回（9:54:34〜35）と 3 回（9:59:39）**押されていた。残り 3 のメールはこの連打の最後（9:54:35）で出た。まとめて取った分を続けて押したのか、意図しない連打かはユーザーに伝えて感想待ち（§12）
...
- **＋1 の連打**（9/24 に SD ログで気づいた）: 1 秒に 3〜5 回の ＋1 がそのまま記録される。意図どおりなら問題なし。誤操作なら「0.5 秒以内の 2 回目は確認を出す」などを検討（§12 の回答待ち）
...
- [x] 「残り 3 のメールが来ない」（9/24）→ 遅れて届いていた。ユーザーが受信を確認
- [x] Wi-Fi 越しの作業の範囲（9/24 回答: **更新＋ログ閲覧、常に受け付ける**）→ 実装済み
- [ ] 9/24 朝の ＋1 の連打（1 秒に 5 回・3 回）は意図どおりか（伝え済み・回答待ち）
