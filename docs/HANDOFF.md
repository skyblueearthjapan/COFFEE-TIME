...
| 通知（GAS・メール） | ✅ デプロイ **@13**（URL 不変。@7〜@13 は AI DUEL・リバーシ・POKER TABLE の受け口の追加だけで、コーヒー側は無変更）。ゲージは端末から届く `max` に連動、文面は「『残り』の数字を長押し」。0 杯の【至急】は見本のみ確認（実機 1→0 は未確認） |
...
- 4 卓: POKER（5 カードドロー・5 ハンド・持ち点 100）/ GOPS（7 or 13）/ THIRTY-ONE（場の 3 枚と交換・3 ハンド・20 手上限）/ BACCARAT（OPEN or CLASSIC・5 回の予想）。相手は JEV（通信）か 端末AI
...
- **初めての人向けの説明**（2026-09-23 ユーザー要望）: 卓ごとに、最初に始めるときだけ「この卓の流れ」＋遊び方のページを自動で出す（前へ `170,379` / 次へ・はじめる `310,379` / スキップ `240,427` /
  ...
  状態表示 `K` の末尾 `seen=1010`（poker/gops/31/bac）。実機で 31・GOPS の説明、切り替え、スキップ、再表示を確認済み
...
`Y!undo`=今日の 1 杯を取り消す（残りは変えない・GAS には送らない）/ `Y!hist,YYYYMMDD,cups`=履歴の 1 日の杯数を直す（合言葉付き。2026-09-23 に誤記録 2 件を消すために使用）/ `X`=探偵の進み具合を初期化。
