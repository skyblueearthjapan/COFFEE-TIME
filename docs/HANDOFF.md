...
- 最終更新: 2026-09-21 深夜（**第 6 版・全面整理**）。最新コミットは `git log` を参照（この文書に固定の番号は書かない）
...
3. **書き込む前に必ず、ユーザーが遊んでいないか確かめる**: `python tools/serlog.py COM8 5 send=GD` で
   `[WOLF] … phase=0` と `[DET] … case=-` なら人狼・探偵は待機中（エスパーには状態表示がまだ無いので、スクリーンショットでも確認）
...
## 3. 現在の状態（2026-09-21 深夜）

| 分野 | 状態 |
|---|---|
| カフェ機能（+1・補充・日付リセット・NVS 保存） | ✅ 運用できる状態。HOME の見出しは「今日」「残り」、天気はマーク＋気温、補充の表示は「補充しました：n 杯」 |
| 通知（GAS・メール） | ✅ デプロイ **@6**（URL 不変）。ゲージは端末から届く `max` に連動、文面は「『残り』の数字を長押し」。0 杯の【至急】は見本のみ確認（実機 1→0 は未確認） |
...
| AI DUEL | 未着手（**次に着手するゲーム**） |
| フラッシュ使用率 | 約 61%（アプリ領域 6.25MB 中 約 4.0MB）。ゲーム用フォント 845 字 |

## 4. 残タスク（優先順）

### これからやる作業
1. **AI DUEL に着手**（ユーザー指示: 次のゲームへ）。設計書 `参考データ/ゲーム4部作/COFFEE_TIME_AI_DUEL_Design_v1.0.md`、要点は [AI_GAMES_DIGEST.md](AI_GAMES_DIGEST.md)。
   最初から GAS 前提の設計だが、PIN・個人履歴まで含みプロジェクト最大の実装。**まずゲストだけ・端末内の統計 AI で遊べる版 → GAS と個人登録 → Jev** の順が現実的。
   着手前に探偵・エスパーと同じく「内容の確認 → ユーザーに要点を説明 → 方針を決めてから実装」の順で進める
2. **エスパー対決 第 2 段階**: 残った候補の順位付けと次の質問選びを Jev に任せる。端末側の差し込み口は `games/esper/core/esper_core.hpp` の `Advisor`。
   GAS には `gas/Jev.gs` の `jevChoice_` がある。**選んだもののメモ（`s_memo`）はサーバーへ絶対に送らない**。
   Jev にすると AI の勝率が上がるはずなので、上がりすぎたら候補の数（`tools/gen_esper_data.py` の `PLAY_MODES`）で調整
3. エスパーの未確認: AI が外れたときの答え合わせ画面、3 分放置のひと休み、開発用シリアルコマンド（**状態表示が無い**ので追加する）、独立したコードレビュー（実装担当が利用上限で最終報告の前に止まったため未実施）
...
- HTTPS は `setInsecure()`（証明書検証なし）。AI DUEL で個人情報（PIN）を扱うなら先に直す
- メインループは Wi-Fi の再接続（最大 10 秒）や GAS への送信（最大 15 秒×2）で止まる。シリアルコマンドと NVS 保存がその間遅れる。ネットワーク処理を別タスクにするのが本筋
...
- **ゲーム一覧の並び順**（2026-09-21 指定）: AI DUEL → エスパー対決 → 喫茶「余白」の事件簿 → 閉店後の人狼会。履歴のゲームの表も同じ順。
  AI DUEL は未実装なので一覧では「準備中」。自動操作のタップ位置: エスパー `240,206` / 探偵 `240,276` / 人狼 `240,346`
...
【PC】COFFEE TIME フォルダー ──git push──▶【GitHub】COFFEE-TIME
   ...
   ▼                                                     ▼
【ESP32】画面・杯数・統計・設定(NVS)・操作ログ(SD)   【Google Apps Script】（スプレッドシートに紐づけ・デプロイ @6）
...
- 疎通試験: `python tools/gas_call.py jevtest`（URL・合言葉・キーは表示されない）
...
| `python tools/uiwalk.py COM8 <dir> key:3 tap:240,367 wait:1 snap:name …` | **画面を自動で操作して順に撮る**（タップは端末の受領の返事を待つ。秘密は開けない） |
...
| `python tools/run_wolf_std_checks.py` / `run_esper_checks.py` | PC 上のロジック試験 |
...
`5`〜`9`=今日の状況・時間ごと・履歴・日ごと・設定 / `B`=明るさ / `F`=時刻合わせ / `I`=システム情報 / `G`=人狼の公開状態 / `D`=探偵の状態 / `X`=探偵の進み具合を初期化。
...
- [ ] AI DUEL の個人登録（PIN）をどこまでやるか（まずゲストだけ、を提案予定）
