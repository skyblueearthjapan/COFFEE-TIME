...
- 最終更新: 2026-09-23 夕方（**第 7 版・全面整理**。第 6 版までの内容をすべて含む）。最新コミットは `git log`（この文書に固定の番号は書かない）
...
### すぐ着手できるもの
0. **残りが 0 のときの「+1」**（ユーザー決定 9/23 夜: **押せるまま + トースト**。次のセッションで実装）: 今の作り = 押せる。「今日」+1、「残り」は 0 のまま、GAS には `left=0` で送る（メールは残りが減った瞬間だけなので出ない）。
   やること: `home::addOneCup()` で `prev == 0` のとき `showToast("残りは 0 のままです。補充したら『残り』を長押し")`（2 秒。`HomeScreen.cpp` の `showToast` は既存）。記録・送信は変えない。理由: 設定 10 杯でも実際は 11〜12 杯出ることがあり、飲んだ杯数の記録を優先する
...
- [x] 残りが 0 のときの「+1」: **押せるまま + トースト**（9/23 夜に回答。§4 の 0 番。次のセッションで実装）
