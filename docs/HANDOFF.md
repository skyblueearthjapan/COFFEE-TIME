...
最終更新: 2026-09-19（最終コミット `23c26ac` 時点）
...
本日の杯数・残り杯数を表示し、残り 3 杯でメール通知する。企画書は `参考データ/`。
...
| 3 | GAS へ記録・残り 3 杯でメール通知 | ✅ 実機で送信〜メール受信まで確認済み（タグ `stage3`） |
...
ESP32 (firmware/)
  ...
  └─ +1・補充・日付変更イベント → HTTPS POST → Google Apps Script (gas/)
                                                 ├─ スプレッドシート「ログ」に追記
                                                 └─ event=take かつ left==3 → 「設定」シートの宛先へ HTML メール
```
...
`T`=+1 / `R`=補充。**`T` と `R` は実際に GAS へ送信され、残り 3 杯になると本当に宛先全員へメールが飛ぶ**ので注意。
...
- 見本メール: `event: "preview"` を正しい TOKEN で POST すると**所有者だけ**に送られる（ログに残らない）
