...
- 最終更新: 2026-09-21（第 4 版）
- 最新コミット: `fe1f785`（ゲーム基盤・人狼 W1 まで反映）
...
| G2 | 人狼 W2（配役・役職確認・議論・投票・結果・覗き見防止） | 🚧 未着手。[WEREWOLF_TODO.md](WEREWOLF_TODO.md) |
| G3 | 探偵 / エスパー / AI DUEL | 未着手（設計書レビュー済み・着手順は人狼→探偵→エスパー→DUEL） |
...
- W1 のファームを書き込み済み。HOME →メニュー→ゲーム一覧→人数選択まで操作できる
- 開発用シリアルコマンドに画面切替（`1`/`2`/`3`/`0`）を追加済み
- 自宅では Wi-Fi 未接続（受信感度の問題）。時刻は RTC と `tools/settime.py` で維持
...
`W`=Wi-Fi スキャン / `C<UNIX秒>`=時刻設定 / `1`=メニュー / `2`=ゲーム一覧 / `3`=人数選択 / `0`=HOME へ
...
ユーザーの回答待ち:
- [ ] **人狼の人数の初期値**: 3 人のままか、4〜5 人にするか、前回の人数を覚えるか（W2 の冒頭で反映したい）
