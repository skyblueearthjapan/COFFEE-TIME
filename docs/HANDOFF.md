...
- 最終更新: 2026-09-22 朝（第 6 版 + **AI DUEL・JEV REVERSI を追加**）。最新コミットは `git log` を参照（この文書に固定の番号は書かない）
...
3. **書き込む前に必ず、ユーザーが遊んでいないか確かめる**: `python tools/serlog.py COM8 5 send=GD` で
   `send=GDVUJ` にすると 5 ゲームぶん出る。`[WOLF] … phase=0`、`[DET] … case=-`、`[ESP] view=0`、`[DUEL] view=-`、`[REV] view=-`（または `paused`）なら待機中
...
| 通知（GAS・メール） | ✅ デプロイ **@9**（URL 不変。@7〜@9 は AI DUEL・リバーシの受け口の追加だけで、コーヒー側は無変更）。ゲージは端末から届く `max` に連動、文面は「『残り』の数字を長押し」。0 杯の【至急】は見本のみ確認（実機 1→0 は未確認） |
...
| フラッシュ使用率 | 約 66%（アプリ領域 6.25MB 中 約 4.30MB）。RAM 26.4%。ゲーム用フォント 868 字 |
...
3. **エスパー対決 第 2 段階**: 残った候補の順位付けと次の質問選びを Jev に任せる。端末側の差し込み口は `games/esper/core/esper_core.hpp` の `Advisor`。
...
- **ゲーム一覧の並び順**（2026-09-21 指定）: AI DUEL → エスパー対決 → 喫茶「余白」の事件簿 → 閉店後の人狼会。履歴のゲームの表も同じ順。
  5 番目にリバーシ（ユーザーの指定は無し。設計書の「第 5 枠」に従った）。**5 段にしたのでタップ位置が変わった**: AI DUEL `240,116` / エスパー `240,178` / 探偵 `240,240` / 人狼 `240,302` / リバーシ `240,364` / もどる `240,428`
...
| `python tools/run_wolf_std_checks.py` / `run_esper_checks.py` / `run_duel_checks.py` / `run_reversi_checks.py` | PC 上のロジック試験 |
...
`5`〜`9`=今日の状況・時間ごと・履歴・日ごと・設定 / `B`=明るさ / `F`=時刻合わせ / `I`=システム情報 / `G`=人狼の公開状態 / `D`=探偵の状態 / `V`=エスパーの状態 / `U`=AI DUEL の状態（相手の手は公開後だけ出る）/ `J`=リバーシの状態 / `X`=探偵の進み具合を初期化。
