...
| G4 | エスパー対決 第 1 段階（端末だけで動くルール方式） | 🚧 実装済み・「ミニ」を実機で最後まで通した（3 問で的中 → 結果）。PC 上の試験 `python tools/run_esper_checks.py` 全合格（手本 168 局と一致、約 58 万件）。**実装担当が利用上限で最終報告の前に止まったため未確認が残る**: レギュラー / フル、外れたときの答え合わせ、きろく画面、3 分放置の一時停止、開発用シリアルコマンド（未追加）、独立したコードレビュー。難易度の呼び名は AI 基準と分かる形に変更（ミニ AIにやさしい / レギュラー いい勝負 / フル AIに手ごわい） |
| G4' | エスパー対決 第 2 段階（Jev に候補の順位付けと次の質問選びを任せる）/ AI DUEL | 未着手。Jev の疎通は確認済み（[AI_GAMES_DIGEST.md](AI_GAMES_DIGEST.md)）。端末側の差し込み口は `games/esper/` の Advisor を参照 |
