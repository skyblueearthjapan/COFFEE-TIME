...
| 分野 | 状態 |
|---|---|
| カフェ機能（+1・補充・日付リセット・NVS 保存） | ✅ 運用できる状態。HOME の見出しは「今日」「残り」、天気はマーク＋気温、補充の表示は「補充しました：n 杯」。**残り 0 で ＋1 すると 2 秒のトースト**（9/23 夜。実機で出る瞬間は未確認） |
...
0b. **残り 0 のトーストの実機確認**: 本物の ＋1 が要るので私は押していない（シートに 1 行残るため）。ユーザーが次にコーヒーを取るとき（9/23 夜の時点で残り 0）に見てもらう。
...
- [ ] 新しいゲームの感想（§4-1）
- [ ] 残り 0 のトーストの目視確認と、毎朝出ることがうるさくないか（§4-0b。9/23 夜に依頼済み）
