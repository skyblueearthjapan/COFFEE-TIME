...
| 通知（GAS・メール） | ✅ デプロイ **@11**（URL 不変。@7〜@11 は AI DUEL・リバーシ・POKER TABLE の受け口の追加だけで、コーヒー側は無変更）。ゲージは端末から届く `max` に連動、文面は「『残り』の数字を長押し」。0 杯の【至急】は見本のみ確認（実機 1→0 は未確認） |
...
4. **エスパー対決 第 2 段階**（ユーザーも「やらないとまずい」と確認 2026-09-23。POKER TABLE の次）: 残った候補の順位付けと次の質問選びを Jev に任せる。端末側の差し込み口は `games/esper/core/esper_core.hpp` の `Advisor`。
...
- 盤は 312×312・左上 (84,84)。タップ位置: 6×6 は `x=110+52×列, y=110+52×行`、8×8 は `x=103+39×列, y=103+39×行`（列・行は 0 から）。マスを選んでから「置く」`240,428`。
  自動操作は「候補」`324,422` → 1 枚目 `180,169` →「置く」で、盤面を知らなくても 1 手打てる。操作 `156,422`。ゲーム一覧のリバーシは `240,364`
...
- ゲーム一覧は 6 段（300×44・間隔 52）: AI DUEL `240,110` / エスパー `240,162` / 探偵 `240,214` / 人狼 `240,266` / リバーシ `240,318` / POKER TABLE `240,370` / もどる `240,428`
